...
import { IconUsersGroup, IconSearch, IconDeviceLaptop, IconFingerprint, IconTag, IconMapPin, IconPencil } from '@tabler/icons-react';
...
export function ResponsablesView({ items, onEdit }: ResponsablesViewProps) {
  ...
  const conResponsable = items.filter((i) => (i.responsable || '').trim());
  ...
  if (!conResponsable.length) {
    return (
      <div className="flex flex-col items-center gap-3.5 px-5 py-15 text-center text-admin-gray">
        ...
        <p className="text-sm">Ningún equipo tiene un responsable asignado todavía.</p>
        <p className="text-xs">Edita un equipo y completa el campo &quot;Responsable&quot; para verlo aquí.</p>
      </div>
    );
  }

  const q = query.toLowerCase().trim();
  const filtered = byPersona.filter(([nombre, list]) => {
    if (!q) return true;
    const searchData = [nombre, ...list.map((i) => `${i.marca || ''} ${i.modelo || ''} ${i.tipo || ''} ${i.serie || ''} ${i.id} ${i.ubicacion || ''} ${i.estado || ''}`)].join(' ').toLowerCase();
    ...
    <div className="flex flex-col gap-3.5">
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        <div className="px-1 text-xs whitespace-nowrap text-admin-gray">
          {byPersona.length} persona{byPersona.length !== 1 ? 's' : ''} · {conResponsable.length} equipo{conResponsable.length !== 1 ? 's' : ''}
        </div>
      </div>

      {filtered.map(([nombre, list]) => (
        <div key={nombre} className="rounded-2xl border-[1.5px] border-admin-border bg-white p-4 dark:border-white/10 dark:bg-admin-dark-surface">
          <div className="mb-2.5 flex items-center gap-2.5">
            <div className="flex h-9.5 w-9.5 shrink-0 items-center justify-center rounded-full bg-admin-blue text-base font-bold text-white">{nombre[0]?.toUpperCase() || '?'}</div>
            <div>
              <div className="text-sm font-bold">{nombre}</div>
              <div className="text-[11px] text-admin-gray">
                {list.length} equipo{list.length !== 1 ? 's' : ''} asignado{list.length !== 1 ? 's' : ''}
              </div>
            </div>
          </div>
          {list.map((item) => {
