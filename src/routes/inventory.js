...
async function getCantidadPrestada(inventarioId) {
...
// GET /api/devices
router.get('/devices', ...requireSuperadminOrAcceso('inventario'), async (req, res) => {
  try {
...
// POST /api/devices
router.post('/devices', ...requireSuperadminOrAcceso('inventario'), async (req, res) => {
  try {
...
// GET /api/inventory
router.get('/inventory', ...requireSuperadminOrAcceso('inventario'), async (req, res) => {
  try {
    ...
    const prestadoPorId = Object.fromEntries(prestados.map(p => [p.inventario_id, p.prestado]));
...
router.patch('/inventory/:id', ...requireSuperadminOrAcceso('inventario'), async (req, res) => {
  try {
...
router.delete('/inventory/:id', ...requireSuperadminOrAcceso('inventario'), async (req, res) => {
  try {
    const id = req.params.id;
...
// GET /api/loans
router.get('/loans', ...requireSuperadminOrAcceso('prestamos'), async (req, res) => {
  try {
...
// PATCH /api/loans/:id
router.patch('/loans/:id', ...requireSuperadminOrAcceso('prestamos'), async (req, res) => {
  try {
    const id   = req.params.id;
    const loan = await db.queryOne('SELECT * FROM prestamos WHERE id = ?', [id]);
    ...
    if (cantidadARegistrar !== null) {
      ...
      const item = await db.queryOne('SELECT tipo_manejo FROM inventario WHERE id = ?', [loan.inventario_id]);
...
// GET /api/auditoria
router.get('/auditoria', ...requireSuperadminOrAcceso('bitacora'), async (req, res) => {
  try {
